...
int OLED_ShowFloat(unsigned char x, unsigned char y, float val, unsigned char TextSize)
{
	uint8_t out[16];
	float_to_char(val,out);
	out[15]='\0';
...
void BackGround(void)
{
    ...
    OLED_ShowStr(104,2,"Hz",2);
    OLED_ShowChar(90,4,'V',2);
    OLED_ShowChar(90,6,'%',2);

//  OLED_ShowStr(0,0,(uint8_t *)"V0",2);
//  OLED_ShowStr(60,0,(uint8_t *)"V1",2);
//  OLED_ShowStr(0,2,(uint8_t *)"V2",2);
//  OLED_ShowStr(60,2,(uint8_t *)"V3",2);
//  OLED_ShowStr(0,4,(uint8_t *)"V4",2);
    OLED_ShowStr(0,6,"THD",2);
}
...
void wave_show (int wave)
{
    switch (wave)
    {
        case sine : OLED_ShowCN(40,0,12);       //正
                    OLED_ShowCN(40+16,0,13);    //弦
                    break;

        case tri :  OLED_ShowCN(40,0,15);       //三
                    OLED_ShowCN(40+16,0,16);    //角
                    break;

        case squ :  OLED_ShowCN(40,0,14);       //方
                    OLED_ShowCN(40+16,0,2);     //波
                    break;
    }
}
